...
public:
	...
	tTVPSoundBuffer(int framesize, SDL_AudioCVT *cvt) : _frame_size(framesize), _cvt(cvt) {
		pthread_mutex_init(&_buffer_mtx, NULL);
	...
	virtual void Reset() override {
		...
		pthread_mutex_lock(&this->_buffer_mtx);
		_buffers.clear();
		_inCachedSamples = 0;
		_sendedFrontBuffer = 0;
		_sendedSamples = 0;
		pthread_mutex_unlock(&this->_buffer_mtx);
	}
	...
	virtual void AppendBuffer(const void *_inbuf, unsigned int inlen/*, int tag = 0*/) override {
		if (_cvt) {
			...
			_buffers.push_back(std::vector<uint8_t>());
			...
			//boost::lock_guard<boost::mutex> lk(_buffer_mtx);
			pthread_mutex_lock(&this->_buffer_mtx);
			_buffers.push_back(std::vector<uint8_t>((uint8_t*)_inbuf, ((uint8_t*)_inbuf) + inlen));
...
protected:
	...
	//boost::mutex _streams_mtx;
	pthread_mutex_t _streams_mtx;
...
public:
	iTVPAudioRenderer() {
		pthread_mutex_init(&_streams_mtx, NULL);
	...
	virtual tTVPSoundBuffer* CreateStream(tTVPWaveFormat &fmt, int bufcount) {
		...
		pthread_mutex_lock(&this->_streams_mtx);
		_streams.insert(s);
		pthread_mutex_unlock(&this->_streams_mtx);
		return s;
	}
...
void TVPInitDirectSound(int freq)
{
	...
	__debugbreak(); throw;
#else
	OutputDebugStringA("==================>TVPInitDirectSound not imp, implemented 20241220 sound good\n");
...
{
	// nothing to do
	OutputDebugStringA("==================>TVPUninitDirectSound not imp, implemented 20241220 sound good\n");
}
...
iTVPSoundBuffer* TVPCreateSoundBuffer(tTVPWaveFormat &fmt, int bufcount)
{
	...
	return NULL;
#else
	OutputDebugStringA("==================>TVPCreateSoundBuffer not imp, implemented 20241220 sound good\n");
	//return NULL;
...
void tTVPSoundBuffer::FillBuffer(uint8_t *out, int len)
{
	...
	while (len > 0 && !_buffers.empty()) {
		std::vector<uint8_t> &buf = _buffers.front();
		if (buf.size() > _sendedFrontBuffer) {
			int n = (size_t)len < (buf.size() - _sendedFrontBuffer) ? (size_t)len : (buf.size() - _sendedFrontBuffer);
			int samples = TVPAudioRenderer->MixAudio(out, &buf.front() + _sendedFrontBuffer, n, _volume_raw);
			_sendedSamples += samples;
			_inCachedSamples -= samples;
			_sendedFrontBuffer += n;
...
void TVPInitDirectSound(int freq)
{
	...
	__debugbreak(); throw;
#else
	fprintf(stderr, "==================>TVPInitDirectSound not imp, implemented 20241220 sound good\n");
...
{
	// nothing to do
	fprintf(stderr, "==================>TVPUninitDirectSound not imp, implemented 20241220 sound good\n");
}
...
iTVPSoundBuffer* TVPCreateSoundBuffer(tTVPWaveFormat &fmt, int bufcount)
{
	...
	return NULL;
#else
	fprintf(stderr, "==================>TVPCreateSoundBuffer not imp, implemented 20241220 sound good\n");
	//return NULL;
	return TVPAudioRenderer->CreateStream(fmt, bufcount);
#endif
}
...
{
#if defined(LINUX)
	printf("==================>TVPInitDirectSound not imp\n");
#else
	//throw;
	__android_log_print(ANDROID_LOG_ERROR, "WaveMixer.cpp", "%s", "==================>TVPInitDirectSound not imp");
#endif
}
...
{
#if defined(LINUX)
	printf("==================>TVPCreateSoundBuffer not imp\n");
#else
	//throw;
	__android_log_print(ANDROID_LOG_ERROR, "WaveMixer.cpp", "%s", "==================>TVPCreateSoundBuffer not imp");
#endif	
	return NULL;
}
...
#else
#error WaveMixer not implemented
