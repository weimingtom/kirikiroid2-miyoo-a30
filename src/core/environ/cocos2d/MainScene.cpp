...
public:
	TVPWindowLayer(tTJSNI_Window *w)
		...
		, _drawTextureScaleX(1.0f)
		, _drawTextureScaleY(1.0f)
		, UseMouseKey(false)
		...
		, Visible(false)
		, LastMouseKeyTick(0)
		, MouseKeyXAccel(0), MouseKeyYAccel(0)
		, LastMouseDownX(0), LastMouseDownY(0) 
		, LastSetImeMode(::imDisable), DefaultImeMode(::imDisable)
		, Closing(false), ProgramClosing(false), CanCloseWork(false)
		, in_mode_(false), modal_result_(0)
	{
		_nextWindow = nullptr;
...
// 		}
	...
	virtual void OnKeyPress(tjs_uint16 vk, int repeat, bool prevkeystate, bool convertkey) override {
		if (TJSNativeInstance && vk) {
			...
			// UNICODE \A4ʤΤǤ\BD\A4Τޤ޶ɤ\B7\A4Ƥ\B7\A4ޤ\A6
			...
			if (b) {
				...
				if (TJSNativeInstance) {
					if (TJSNativeInstance->IsMainWindow()) {
						...
						iTJSDispatch2 * obj = TJSNativeInstance->GetOwnerNoAddRef();
						obj->Invalidate(0, NULL, NULL, obj);
						// TJSNativeInstance = NULL; // \A4\B3\A4ζ\CE\EBA\A4ǤϼȤ\CBthis\A4\AC\CF\F7\B3\FD\A4\B5\A4\EC\A4Ƥ\A4\A4뤿\A4ᡢ\A5\E1\A5\F3\A5Щ`\A4إ\A2\A5\AF\A5\BB\A5\B9\A4\B7\A4ƤϤ\A4\A4\B1\A4ʤ\A4
					}
				} else {
					delete this;
				}
