...
static void ClearDumps(const std::string &dumpdir, std::vector<std::string> &allDumps) {
	...
	for (auto p_path = allDumps.begin(); p_path != allDumps.end(); ++p_path)
	{
...
static std::string url_encode(const std::string &value) {
	...
	for (auto p_c = value.begin(); p_c != value.end(); ++p_c)
	{
		const auto& c = *p_c;
...
static void SendDumps(std::string dumpdir, std::vector<std::string> allDumps, std::string packageName, std::string versionStr) {
	...
	for (auto p_filename = allDumps.begin(); p_filename != allDumps.end(); ++p_filename)
	{
		const auto& filename = *p_filename;
		...
		for (auto p_it = _inmemFiles.begin(); p_it != _inmemFiles.end(); ++p_it)
		{
			auto& it = *p_it;
			delete it.second;
		}
