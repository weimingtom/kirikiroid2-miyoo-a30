...
//---------------------------------------------------------------------------
// \8C\9F\8D\F5\82\BE\82\AF\82\B7\82\E9
...
#if defined(ANDROID)
		__android_log_print(ANDROID_LOG_ERROR, "GraphicsLoaderIntf.cpp", "%s", "*** *** TVPGraphicPreload");
#endif
	...
	tjs_uint64 timeout)
{
	...
	if (/*!timeout &&*/ storages.size()/* > 1*/) { // using async touching for multi images
		...
		for (auto p_name = storages.begin(); p_name != storages.end(); ++p_name)
		{
			const auto& name = *p_name;
			ttstr nname = TVPNormalizeStorageName(name);
			tjs_uint32 hash;
			tTVPGraphicsSearchData searchdata;
			if (TVPGraphicCacheEnabled) {
