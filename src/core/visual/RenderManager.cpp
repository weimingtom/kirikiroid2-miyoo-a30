...
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
//FIXME:TAffuncFunc not support more than 10 params in vs2010
//std::pair<tjs_int, tjs_int> sxs_sys(sxs, sys);
...
void iTVPTexture2D::RecycleProcess()
{
	...
	for (auto p_tex = _toDeleteTextures.begin(); p_tex != _toDeleteTextures.end(); ++p_tex)
	{
		auto& tex = *p_tex;
		delete tex;
	}
...
protected:
	...
	tTVPSoftwareTexture2D_compress(int pitch, unsigned int w, unsigned int h, TVPTextureFormat::e format)
		: tTVPSoftwareTexture2D_static(nullptr, pitch, w, h, format), PixelFrameLife(0) {}
...
public: tTVPRenderMethod_Software() : _nameHash(0) {}
public:
...
public:
	tTVPRenderMethod_TransBlt(): ParameterIDBegin(0), ParameterIDEnd(0 + 1) {}
	virtual void DoRender(
		iTVPTexture2D *_tar, const tTVPRect &rctar,
		iTVPTexture2D *_dst, const tTVPRect &rcdst,
		iTVPTexture2D *_src, const tTVPRect &rcsrc,
...
struct tRenderMethodCache {
	struct MethodInfo {
		MethodInfo():Normal(nullptr), HDA(nullptr), WithOpa(nullptr), WithOpaHDA(nullptr) {}
...
#else
#define TVP_DoAffineLoop_ARGS  sxs_sys, \
	dest, l, len, src, srcpitch, sxl, syl, srcrect
#define TVP_DoBilinearAffineLoop_ARGS  sxs_sys, \
	dest, l, len, src, srcpitch, sxl, syl, srccliprect, srcrect

	typedef std::function < void(
		const std::pair<tjs_int, tjs_int>&, tjs_uint8*, tjs_int, tjs_int, const tjs_uint8 *,
		tjs_int, tjs_int, tjs_int, const tTVPRect &, const tTVPRect &) > TAffuncFunc;
#endif
...
// 					};
		...
		throw;
#elif defined(_MSC_VER)	
		OutputDebugStringA("=====================>GetStretchFunction not imp\n");
		__debugbreak(); throw;
#else
		throw;
#endif	
...
#else
#if defined(_MSC_VER)
	OutputDebugStringA("=========================TVPIsSoftwareRenderManager return null\n");
#else
	CCLOG("=========================TVPIsSoftwareRenderManager return null");
#endif
	return NULL;
#endif
}
...
bool TVPIsSoftwareRenderManager() {
	...
	//return ret;
#if defined(_MSC_VER)
	OutputDebugStringA("=========================TVPIsSoftwareRenderManager return true\n");
#else
	CCLOG("=========================TVPIsSoftwareRenderManager return true");
#endif
	return true;
#endif
}
...
#else
#if defined(_MSC_VER)
	OutputDebugStringA("=========================RecycleProcess\n");
#else
	CCLOG("=========================RecycleProcess");
#endif
#endif
}
