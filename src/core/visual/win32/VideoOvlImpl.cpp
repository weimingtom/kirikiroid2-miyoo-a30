...
: EventQueue(this,&tTJSNI_VideoOverlay::WndProc), CachedOverlay(nullptr)
{
	VideoOverlay = NULL;
	Rect.left = 0;
	Rect.top = 0;
				...
				switch( evcode )
				{
					...
					case EC_UPDATE:
						if( Mode == vomLayer && Status == ssPlay )
						{
							...
							// 2\83t\83\8C\81[\83\80\88ȏ㍷\82\AA\82\A0\82\E9\82Ƃ\AB\82\CDGetFrame() \82\F0\8C\BB\8D݂̃t\83\8C\81[\83\80\82Ƃ\B7\82\E9
							int frame = GetFrame();
							...
							if( (!IsPrepare) && (SegLoopEndFrame > 0) && (frame >= SegLoopEndFrame) ) {
								...
								return; // Update\82\F0\8Ds\82\ED\82Ȃ\A2
							}
							...
							else	// 0\82\B6\82\E1\82Ȃ\A9\82\C1\82\BD\82\E7\81A1\82Ƃ݂Ȃ\B7\81B
							{
...
{
	// \83r\83f\83I\83I\81[\83v\83\93\8C\E3\82̃\82\81[\83h\95ύX\82͋֎~
	if( !VideoOverlay )
	{
		Mode = m;
	}
	...
	if(VideoOverlay)
	{
		if( l )
		{
			if( l->GetVisible() )
			{
				...
				if( bmp )
				{
					// \8E\A9\91O\82\C5DC\82\F0\8D\EC\82\E9
					HDC hdc;
					HDC			ref = GetDC(0);
